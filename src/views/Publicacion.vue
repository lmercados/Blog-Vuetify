...
export default {
  ...
  mounted() {
    this.mostrarLoading({
      titulo: "Accciendo a la Información del Post Post.....",
      estado: true
    });
    axios
      .get(
        `https://jsonplaceholder.typicode.com/posts/${this.$route.params.id_publicacion}`
      )
      .then(response => {
      ...
      .finally(() => {
        axios
          .get(
            `https://jsonplaceholder.typicode.com/posts/${this.$route.params.id_publicacion}/comments`
          )
          .then(response => {
